...
VK_CODE = {
  ...
  'with_shift_|':0xDC, 'with_shift_@':0x32, 'with_shift_!':0x31, 'with_shift_#':0x33, 
  'with_shift_$':0x34, 'with_shift_%':0x35, 'with_shift_^':0x36, 'with_shift_&':0x37, 
  'with_shift_*':0x38
  }
with_shitf_word = ['|', '!', '@', '#', '$', '%', '^', '&', '*']
class Command:
	...
	u'''按下键'''
	def passonekey(self, word):
		if(word.find('with_shift_') != -1):
	...
	def change_word(self, word):
		if(word == " "):
			word = "spacebar"
		if(word in with_shitf_word):
			word = "with_shift_" + word
		return word	
		
	u'''string类型转换为array'''
	def string_to_array(self, cmd):
		cmdlist = list()
		for i in range(len(cmd)):
			cmdlist.append(self.change_word(cmd[i]))
		return cmdlist
		
	u'''执行命令'''
	def excute(self, cmd):
		cmdlist = self.string_to_array(cmd)
		for i in range(len(cmdlist)):
			self.passonekey(cmdlist[i])
		self.peration_key("enter")